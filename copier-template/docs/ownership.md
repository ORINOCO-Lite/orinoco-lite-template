...
| `consumer_tests` | The site after one-time creation | browser, integration, offline, parity, and contract tests |
| `site_policy` | The site | license, citation, contribution, and conduct files |
| `generated` | Declared generators after inputs validate | projection and manifests under `generated/` |
...
The updater hashes every protected site-owned byte and the complete committed generated projection before and after its run.
Only `generated/manifests/framework-update.json` may change as ordinary updater bookkeeping.
...
The site owns every tracked byte below `tests/browser/`, including the npm manifest and lock.
