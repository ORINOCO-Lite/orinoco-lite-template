...
The `github-template` branch is a derived distribution for GitHub's repository template feature.
It is rendered from an exact source tag and is not used as a test fixture.
See [testing](docs/testing.md), [publishing](docs/publishing.md), and [releasing](docs/releasing.md).
...
The first build resolves the exact upstream presentation, so it needs network access to GitHub and takes longer than later builds.
...
`verify-build` builds twice and compares the trees, so it also proves the site is reproducible.

One immutable `orinoco-lite` wheel contains the code, schema, static interfaces, and licenses.
The `package` mapping in `orinoco.lock` records its `version`, `url`, and `sha256`; Copier records the same selection as `package_version`, `package_url`, and `package_sha256`.
