...
- Add the optional `pr_previews` Copier question and its single `netlify` provider.
  Selecting it renders a pull-request-only `netlify.toml` that installs the reviewed Pixi release and runs the repository's ordinary root-relative build, without touching the canonical GitHub Pages deployment or adding preview-specific tooling.
